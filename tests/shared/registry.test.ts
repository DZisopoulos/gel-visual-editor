...
describe('registry', () => {
  ...
  it('throws on unknown type', () => {
    expect(() => getNodeDef('nope')).toThrow()
  })
  ...
  it('set-variable generates core:set and introduces its var', () => {
    const blk = {
      ...createBlock('set-variable'),
      props: { stepName: '', varName: 'total', value: '${rows.rowCount}' }
    }
  ...
  it('for-each wraps rendered children indented', () => {
    const blk = {
      ...createBlock('for-each'),
      props: { stepName: '', items: '${rows.rows}', varName: 'row' }
    }
