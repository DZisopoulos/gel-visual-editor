...
// fresh on each open instead of being reset from an effect.
function CommandPaletteBody({ onClose }: Omit<CommandPaletteProps, 'open'>): React.JSX.Element {
  ...
  const items = useMemo<CommandItem[]>(() => {
    ...
    if (pastLength > 0)
      actions.push({ id: 'undo', label: 'Undo last change', detail: 'Ctrl/Cmd + Z', run: undo })
    if (futureLength > 0)
      actions.push({
        id: 'redo',
        label: 'Redo last change',
        detail: 'Ctrl/Cmd + Shift + Z',
        run: redo
      })
    ...
    return [...actions, ...blocks].filter(
      (item) => !normalized || `${item.label} ${item.detail}`.toLowerCase().includes(normalized)
    )
  }, [addBlock, flow.blocks.length, futureLength, pastLength, query, redo, undo])

  const choose = (item: CommandItem | undefined): void => {
    if (item) {
      item.run()
      onClose()
    }
