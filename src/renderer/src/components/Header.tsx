...
function Header(): React.JSX.Element {
  ...
  const suggestedName = (): string =>
    flow.meta.name.replace(/[<>:"/\\|?*]/g, '-').trim() || 'Untitled Flow'

  const handleSave = async (): Promise<void> => {
    try {
      const existingGvePath = filePath?.toLowerCase().endsWith('.gve') ? filePath : null
      const savedPath = await window.gve.saveFlow(
        suggestedName(),
        serializeFlow(flow),
        existingGvePath
      )
      if (savedPath) {
        markSaved(savedPath)
        push('Flow saved successfully.', 'success')
      }
    } catch (error) {
      await alert(
        'Save failed',
        `Could not save this flow. ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
  ...
  const handleOpen = async (): Promise<void> => {
    if (
      dirty &&
      !(await confirm('Open another flow?', 'Discard your unsaved changes and open another flow?', {
    ...
    try {
      ...
      if (result.drift) {
        await alert(
          'XML changed',
          'This XML was changed after export. GVE restored the embedded flow definition.'
        )
      }
    } catch (error) {
      await alert(
        'Open failed',
        `Could not open this flow. ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
  ...
  const handleExport = async (): Promise<void> => {
    ...
    if (errors.length > 0) {
      await alert(
        'Export blocked',
        `${errors.length} validation error${errors.length === 1 ? '' : 's'} must be fixed first.`
      )
      return
    }
    if (
      ...
      !(await confirm(
        ...
        `This flow has ${warnings.length} warning${warnings.length === 1 ? '' : 's'}. Export anyway?`,
    ...
    try {
      ...
      await alert(
        'Export failed',
        `Could not export this flow. ${error instanceof Error ? error.message : String(error)}`
      )
    }
  }
  ...
  return (
    <header className="gve-header">
      ...
      />
      <div
        className={`gve-flow-status${dirty ? ' gve-flow-status-dirty' : ''}${errorCount > 0 ? ' gve-flow-status-error' : ''}`}
        aria-label={
          errorCount > 0
      ...
      <div className="gve-header-actions">
        <button
          type="button"
          className="gve-icon-button gve-icon-undo"
          aria-label="Undo"
          title="Undo"
          onClick={undo}
        ...
        <button
          type="button"
          className="gve-icon-button gve-icon-redo"
          aria-label="Redo"
          title="Redo"
          onClick={redo}
        ...
        <button
          type="button"
          className="gve-icon-button gve-icon-open"
        ...
        <button
          type="button"
          className="gve-icon-button gve-icon-save"
        ...
        <button
          type="button"
          className="gve-icon-button gve-icon-export"
          aria-label="Export GEL"
          title="Export GEL"
