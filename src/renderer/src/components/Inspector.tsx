...
function fieldClass(field: FieldDef, value: string): string {
  ...
  if (field.kind === 'sql' || field.kind === 'xml' || field.kind === 'expression') classes.push('mono')
...
function Inspector(): React.JSX.Element {
  ...
  const updateParameter = (index: number, patch: Partial<FlowParameter>): void => {
    updateParameters(flow.parameters.map((parameter, i) =>
      ...
      <aside className="gve-inspector" aria-label="Inspector">
        ...
        <div className="gve-inspector-body">
          ...
          <label className="gve-field">
            ...
            <textarea value={flow.meta.description} onChange={event => updateMeta({ description: event.target.value })} />
          </label>
          <label className="gve-field">
            <span>Script type</span>
            <select value={flow.meta.scriptType} onChange={event => updateMeta({ scriptType: event.target.value as 'process-step' | 'standalone' })}>
          ...
          <div className="gve-parameters-head">
            ...
            <button
              type="button"
              onClick={() => updateParameters([...flow.parameters, { name: '', type: 'string', default: '' }])}
            ...
            {flow.parameters.map((parameter, index) => (
              <div className="gve-parameter-row" key={index}>
                ...
                <select
                  ...
                  onChange={event => updateParameter(index, { type: event.target.value as FlowParameter['type'] })}
                >
            ...
            <label className="gve-field" key={field.key}>
              ...
              {field.kind === 'select' ? (
                <select
                  className={className}
                  value={value}
                  onChange={event => updateProps(selected.id, { [field.key]: event.target.value })}
                >
