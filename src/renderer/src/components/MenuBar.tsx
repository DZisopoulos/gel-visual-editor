...
function MenuBar({
  ...
  const newFlow = async (): Promise<void> => {
    if (
      dirty &&
      !(await confirm('Start a new flow?', 'Discard your unsaved changes and start a new flow?', {
      ...
      <div className="gve-menu-groups">
        {(['file', 'edit', 'view', 'help'] as const).map((menu) => (
          <div className="gve-menu" key={menu}>
            <button
              type="button"
              className={`gve-menu-trigger${openMenu === menu ? ' gve-menu-trigger-open' : ''}`}
              aria-haspopup="true"
