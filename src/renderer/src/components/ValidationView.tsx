...
function ValidationView({ onOpenFlow }: ValidationViewProps): React.JSX.Element {
  ...
  useEffect(() => {
    const timer = window.setTimeout(() => {
      ...
      push(errorCount === 0 ? 'Validation complete: flow is ready to export.' : `Validation complete: ${errorCount} error${errorCount === 1 ? '' : 's'} found.`, errorCount === 0 ? 'success' : 'error')
    }, 180)
    ...
    <section className="gve-validation-view" aria-label="Validation view">
      <header className="gve-validation-header">
        ...
        <div className="gve-validation-summary" aria-label="Validation summary">
          <span className="gve-validation-pill gve-validation-error"><span className="gve-validation-pill-icon" aria-hidden="true">!</span>{counts.error} errors</span>
          <span className="gve-validation-pill gve-validation-warning"><span className="gve-validation-pill-icon" aria-hidden="true">△</span>{counts.warning} warnings</span>
          <span className="gve-validation-pill gve-validation-info"><span className="gve-validation-pill-icon" aria-hidden="true">i</span>{counts.info} notes</span>
        </div>
      </header>
      {isValidating ? <div className="gve-validation-skeleton" aria-label="Validating flow" aria-busy="true"><span /><span /><span /><span /></div> : issues.length === 0 ? <div className="gve-validation-empty"><span aria-hidden="true">✓</span><h3>Everything looks good</h3><p>Your flow has no detected validation issues.</p></div> : (
