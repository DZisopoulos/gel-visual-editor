...
export function parseFlowDocument(value: unknown): Flow {
  if (!isRecord(value) || value.gveVersion !== '1.0' || !isRecord(value.meta) || !Array.isArray(value.parameters) || !Array.isArray(value.datasources) || !Array.isArray(value.blocks)) throw new Error('Flow file has an unexpected shape.')
  const meta = value.meta as Partial<FlowMeta>
  if (typeof meta.name !== 'string' || typeof meta.description !== 'string' || (meta.scriptType !== 'process-step' && meta.scriptType !== 'standalone')) throw new Error('Flow metadata is invalid.')
  const parameters: FlowParameter[] = value.parameters.map(parameter => {
    if (!isRecord(parameter) || typeof parameter.name !== 'string' || typeof parameter.default !== 'string' || !['string', 'number', 'date'].includes(String(parameter.type))) throw new Error('A flow parameter is invalid.')
    return { name: parameter.name, type: parameter.type as FlowParameter['type'], default: parameter.default }
  })
  if (value.datasources.some(datasource => typeof datasource !== 'string')) throw new Error('Flow datasources are invalid.')
  return { gveVersion: '1.0', meta: { name: meta.name, description: meta.description, scriptType: meta.scriptType }, parameters, datasources: value.datasources as string[], blocks: value.blocks.map(parseBlock) }
}
