...
export function variablesInScope(flow: Flow, targetId: string): string[] {
  ...
  const walk = (blocks: Block[], available: Set<string>): string[] | null => {
    for (const block of blocks) {
      ...
      let definition
      try { definition = getNodeDef(block.type) } catch { continue }
      const next = new Set(available)
      definition.introduces(block.props).filter(Boolean).forEach(variable => next.add(variable))
      if (block.children) {
        const nested = walk(block.children, next)
        if (nested) return nested
      }
      definition.introduces(block.props).filter(Boolean).forEach(variable => available.add(variable))
    }
    return null
  }
...
function label(block: Block): string {
  try { return block.props.stepName || getNodeDef(block.type).name } catch { return block.type }
}
...
export function validateFlow(flow: Flow): ValidationIssue[] {
  ...
  const walk = (blocks: Block[], available: Set<string>): void => {
    for (const block of blocks) {
      let definition
      try { definition = getNodeDef(block.type) } catch {
        add({ severity: 'error', title: 'Unknown block type', message: `The block type “${block.type}” is not registered.`, blockId: block.id })
        continue
      }
      const blockName = label(block)
      for (const field of definition.fields) {
        const value = block.props[field.key] ?? ''
        if (field.required && !value.trim()) add({ severity: 'error', title: 'Required field is empty', message: `${field.label} is required on “${blockName}”.`, blockId: block.id })
        if (field.kind === 'expression' && value.trim()) {
          for (const variable of referencedVariables(value)) {
            if (!available.has(variable)) add({ severity: 'warning', title: 'Variable may be out of scope', message: `“${variable}” is referenced before it is introduced.`, blockId: block.id })
          }
        }
      }
      if (!block.enabled) add({ severity: 'info', title: 'Block is disabled', message: `“${blockName}” will be exported as a comment.`, blockId: block.id })
      if (definition.isContainer && (block.children?.length ?? 0) === 0) add({ severity: 'warning', title: 'Empty container', message: `“${blockName}” has no child blocks.`, blockId: block.id })

      const introduced = definition.introduces(block.props).filter(Boolean)
      for (const variable of introduced) {
        if (available.has(variable)) add({ severity: 'warning', title: 'Variable is already in scope', message: `“${variable}” is introduced again by “${blockName}”.`, blockId: block.id })
      }
  ...
  if (!flow.meta.name.trim()) add({ severity: 'warning', title: 'Flow has no name', message: 'Give the flow a name before exporting.' })
  return issues
}
