...
export function findBlock(blocks: Block[], id: string): Block | null {
  for (const blk of blocks) {
    ...
    if (blk.children) {
      const hit = findBlock(blk.children, id)
      if (hit) return hit
    }
  }
  return null
...
export function insertBlock(blocks: Block[], block: Block, target: DropTarget): Block[] {
  if (target.parentId === null) {
    const out = [...blocks]
    out.splice(target.index, 0, block)
    return out
  }
  let found = false
  const walk = (list: Block[]): Block[] =>
    list.map((blk) => {
      if (blk.id === target.parentId) {
        ...
        kids.splice(target.index, 0, block)
...
export function moveBlock(blocks: Block[], id: string, target: DropTarget): Block[] {
  ...
  const adjustedTarget =
    source.parentId === target.parentId && source.index < target.index
      ...
      : target
  if (source.parentId === adjustedTarget.parentId && source.index === adjustedTarget.index)
    return blocks
