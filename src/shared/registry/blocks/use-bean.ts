...
export const useBean: NodeDefinition = {
  type: 'use-bean', name: 'Use Bean', category: 'core', color: '#A78BFA',
  namespaces: ['core'],
  fields: [
  ...
  toGel: block => [`<core:useBean var="${escapeAttr(block.props.varName)}" class="${escapeAttr(block.props.className)}"/>`]
}
